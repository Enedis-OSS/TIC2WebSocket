 ...
 *
 * <p>This class extends {@link ChannelPhysical} and implements {@link SerialPortEventListener} to
 * provide a robust serial port communication channel. It handles automatic port discovery,
 * configuration, connection management, and real-time data transmission for TIC-compliant devices.
 *
 * <p>Key features include:
 *
 * <ul>
 *   <li>Automatic port detection and reconnection when devices are plugged/unplugged
 *   <li>Support for both port ID and port name identification methods
 *   <li>Configurable serial communication parameters (baud rate, parity, data bits, stop bits)
 *   <li>Event-driven data reception with optional timeout handling
 *   <li>Symbolic link resolution for Linux systems
 *   <li>Error detection and automatic recovery mechanisms
 * </ul>
 *
 * <p>The channel operates in a periodic polling mode to monitor port availability and automatically
 * handles port changes, busy conditions, and communication errors. It supports both synchronous and
 * asynchronous data operations with proper exception handling.
 *
...
public class ChannelSerialPort extends ChannelPhysical implements SerialPortEventListener {
   ...
   *
   * <p>This method queries the system to determine if a port with the specified identifier or name
   * is currently available. The port ID takes precedence over port name if both are provided.
   *
   ...
   * parameter takes priority over the port name parameter if both are provided. This is useful for
   * converting abstract port identifiers to concrete system paths.
   *
   ...
   *
   * <p>This utility method maps the application-level parity enumeration to the underlying serial
   * communication library constants required for port configuration.
   *
   ...
   *
   * <p>This utility method maps the configured stop bits value (1.0, 1.5, or 2.0) to the underlying
   ...
   *
   * <p>This constructor initializes the channel with the provided configuration and sets up the
   * initial port detection. The channel will automatically resolve the port name based on the
   * configuration's port ID or port name settings.
   *
   * <p>The constructor sets up the periodic polling mechanism with a default delay and initializes
   * the port change detection system.
   *
   ...
   * @throws IllegalArgumentException if configuration is null or not a
   ...
   *
   * <p>This method initiates the channel by opening the serial port connection and starting the
   * underlying periodic task. The port is opened with the configuration parameters specified during
   * construction.
   *
   * <p>If the port cannot be opened, the channel will enter an error state and attempt to reconnect
   * periodically.
   *
   * @see #stop()
   * @see #openPort()
   ...
   *
   * <p>This method gracefully stops the channel by first stopping the underlying periodic task and
   * then closing the serial port connection. All event listeners are removed and the port is
   * properly released.
   *
   ...
   *
   * <p>This method validates and applies a new configuration to the channel. The configuration must
   * be a valid {@link ChannelSerialPortConfiguration} instance. If the channel is currently
   * started, it will be stopped and restarted with the new configuration.
   *
   * <p>The configuration update includes port parameter changes, which may require reopening the
   * serial port connection.
   *
   * @param configuration the new channel configuration to apply
   * @throws ChannelException if the configuration is null, invalid, or not a serial port
   *     configuration
   * @see ChannelSerialPortConfiguration
   */
  @Override
  public void setup(ChannelConfiguration configuration) throws ChannelException {
    ...
    if (!(configuration instanceof ChannelSerialPortConfiguration)) {
      ChannelException.raiseInvalidConfigurationType(
          configuration, ChannelSerialPortConfiguration.class.getSimpleName());
    }
   ...
   *
   * <p>This method performs a non-blocking read operation to retrieve all available data from the
   * serial port input buffer. The method returns immediately with whatever data is currently
   * available, or an empty array if no data is present.
   *
   * <p>The channel must be in a STARTED state and the port must be open for this operation to
   * succeed.
   *
   ...
   *
   * <p>This method transmits the provided data bytes to the serial port. The operation is
   * synchronous and will block until all data has been written to the port buffer.
   *
   * <p>The channel must be in a STARTED state and the port must be open for this operation to
   * succeed.
   *
   * @param data the byte array to write to the port (must not be null)
   * @throws ChannelException if the channel is not ready, port is not open, or write operation
   *     fails
   ...
   *
   * <p>This method returns a defensive copy of the channel's configuration to prevent external
   * modification of the internal state. The returned configuration contains all current settings
   * including port identification, communication parameters, and timeout values.
   *
   ...
   *
   * <p>This protected method handles the internal setup process when the channel configuration is
   * updated. It saves the current status, stops the channel, updates the real path for symbolic
   * links on Linux systems, creates a new port handler if needed, and restores the previous state.
   *
   * <p>This method ensures that configuration changes are applied atomically and that the channel
   * state is properly maintained throughout the update process.
   *
  ...
  protected void setup() throws ChannelException {
    ...
    if ((null == this.portHandler)
        || (false == this.getPortName().equals(this.portHandler.getPortName()))) {
   ...
   *
   * <p>This method is called by the JSSC library when serial port events occur. Currently, it only
   * handles RXCHAR events (data received) by triggering the data reception process. Other event
   * types are ignored as they are not relevant for TIC communication.
   *
   * <p>The event handling is designed to be lightweight and delegates the actual data processing to
   * the {@link #receiveData()} method.
   *
   ...
   *
   * <p>This method is called periodically to monitor the port status and handle various scenarios
   * including port detection, connection management, and data reception.
   *
   * <p>When the channel is STARTED, it:
   *
   * <ul>
   *   <li>Checks if the port is still available and handles port not found scenarios
   *   <li>Detects port name changes (e.g., due to USB device reconnection)
   *   <li>Receives data if no event listener is configured (polling mode)
   * </ul>
   *
   * <p>When the channel is not STARTED, it attempts to open the port if it becomes available, with
   * extended delay on error conditions.
   *
   * <p>The processing delay is dynamically adjusted based on the current state and error conditions
   ...
   *
   * <p>This method reads exactly the specified number of bytes from the serial port. The operation
   * will block until the requested number of bytes is available or until the operation times out.
   *
   * <p>The channel must be in a STARTED state and the port must be open for this operation to
   * succeed.
   *
   ...
   *
   * <p>This method reads exactly the specified number of bytes from the serial port within the
   * given timeout period. If the timeout expires before the requested number of bytes is available,
   ...
   * @return array containing the read bytes, or null if the read operation fails
   * @throws ChannelException if the channel is not ready, port is not open, read operation fails,
   *     or timeout occurs
   ...
   *
   * <p>This method queries the serial port input buffer to determine how many bytes are currently
   * available for reading. This information is useful for determining whether data is available
   * before performing a read operation.
   *
   * <p>The channel must be in a STARTED state and the port must be open for this operation to
   * succeed.
   *
   ...
   *
   * <p>This method clears both the input and output buffers of the serial port, discarding any
   * pending data. This is useful for ensuring a clean communication state, particularly after
   * errors or before starting a new communication session.
   *
   * <p>The flush operation includes clearing receive buffers, transmit buffers, and aborting any
   * pending operations.
   *
   * <p>The channel must be in a STARTED state and the port must be open for this operation to
   * succeed.
   *
   * @throws ChannelException if the channel is not ready, port is not open, or flush operation
   *     fails
   ...
   *
   * <p>The port ID is an alternative way to identify serial ports, typically used when the system
   * provides unique identifiers for ports. This identifier takes precedence over the port name when
   * both are specified.
   *
   ...
   *
   * <p>The port name is the system path or name used to identify the serial port. This is typically
   * a device path like "/dev/ttyUSB0" on Linux or "COM1" on Windows.
   *
   ...
   *
   * <p>This method returns the actual port name that is currently opened by the JSSC library. This
   * may differ from the configured port name if symbolic links were resolved or if the port was
   ...
   *
   * <p>This method resolves the port name based on the configuration's port ID or port name
   * settings. It uses the same resolution logic as the static {@link #findPortName(String, String)}
   * method but operates on the current configuration.
   *
   * @return the resolved port name, or null if no matching port is found
   ...
   *
   * <p>Data bits define the number of bits used to represent each character transmitted. Standard
   * values are 7 (for ASCII) or 8 (for binary data). TIC protocol typically uses 7 data bits.
   *
   ...
   *
   * <p>Stop bits mark the end of each data frame. Standard values are 1.0, 1.5, or 2.0. Most serial
   * communications use 1.0 stop bits.
   *
   ...
   *
   * <p>Parity is used for error detection by adding an extra bit to each data frame. TIC protocol
   * typically uses EVEN parity for reliable data transmission.
   *
   ...
   *
   * <p>This timeout determines how long a synchronous read operation will wait for data before
   * timing out. The value is specified in milliseconds.
   *
   ...
   *
   * <p>This method indicates whether the channel is configured to use event-driven data reception
   * (via SerialPortEventListener) or polling-based reception.
   *
   * <p>The base implementation returns false, indicating polling mode. Subclasses can override this
   * method to enable event-driven mode.
   *
   ...
   *
   * <p>This method configures the serial port to generate events when data is received, control
   * signals change, or other port events occur. It sets up the event mask to listen for RXCHAR
   * (data received), CTS (clear to send), and DSR (data set ready) events.
   *
   * <p>The channel instance is registered as the event listener, so {@link
   * #serialEvent(SerialPortEvent)} will be called when events occur.
   *
  ...
  protected boolean addEventListener() {
    ...
    try {
      if (!this.portHandler.setEventsMask(mask)) {
        this.logger.error(
            "Channel "
                + this.getName()
                + " failed to set event mask on port "
                + this.getPortName()
                + " ("
                + SystemError.getMessage()
                + ") ");
    ...
    try {
      ...
      this.logger.error(
          "Channel " + this.getName() + " failed to add listener :" + e.getMessage(), e);
      return false;
    }
   ...
   *
   * <p>This method unregisters the channel instance as an event listener from the serial port.
   * After calling this method, the channel will no longer receive asynchronous events and will
   * operate in polling mode.
   *
   * <p>This method is typically called when stopping the channel or switching to polling-based data
   * reception.
   *
  ...
  protected boolean removeEventListener() {
    ...
    try {
      ...
      this.logger.error(
          "Channel " + this.getName() + " failed to remove listener :" + e.getMessage(), e);
      return false;
    }
   ...
   *
   * <p>This method checks for available data in the serial port input buffer and reads it if
   * present. The received data is then passed to registered listeners via the notification system.
   *
   * <p>If no data is available, the method returns immediately without error. If a read error
   * occurs, it notifies listeners with an appropriate error code.
   *
   * <p>This method is called both from the periodic processing loop (polling mode) and from the
   * serial event handler (event-driven mode).
   *
   * @see #read()
   * @see #available()
   ...
   *
   * <p>This method compares the currently detected port name with the previously detected port name
   * to determine if the port has changed. This typically occurs when a USB device is unplugged and
   * plugged back in, causing the system to assign a new device path.
   *
   * <p>The comparison is case-insensitive to handle variations in how the system reports port
   * names.
   *
  ...
  private boolean hasPortChanged() {
    ...
    if (this.currentPortName != null
        && !this.currentPortName.equalsIgnoreCase(this.previousPortName)) {
      portChanged = true;
    }
   ...
   *
   * <p>This method is called when a port name change is detected. It logs the change, sets the
   * channel status to ERROR, forcibly closes the current port connection, notifies listeners of the
   * error, and updates the previous port name for future comparisons.
   *
   * <p>Port name changes typically occur when USB devices are reconnected and the system assigns a
   * new device path (e.g., /dev/ttyUSB0 → /dev/ttyUSB1).
   *
   * @see #hasPortChanged()
  ...
  private void onPortNameChanged() {
    String errorMessage =
        "Port name has changed ( " + this.previousPortName + " --> " + this.currentPortName + " )";
    this.logger.error(errorMessage);
    this.setStatus(ChannelStatus.ERROR);
    this.closePortForced();
    this.notifyOnErrorDetected(
        ChannelSerialPortErrorCode.PORT_NAME_HAS_CHANGED.getCode(ERROR_CODE_OFFSET), errorMessage);
    this.previousPortName = this.currentPortName;
  }
   ...
   *
   * <p>This method is only executed on Linux systems to resolve symbolic links to their actual
   * device paths. This is important for USB devices that may be represented by symbolic links in
   * /dev/disk/by-id/ or similar locations.
   *
   * <p>The method uses the system 'realpath' command to resolve the symbolic link and updates the
   * channel configuration with the resolved path.
   *
    ...
    if (SystemUtils.IS_OS_LINUX) {
      ...
      try {
        ...
        throw new ChannelException(
            ChannelException.ERRCODE_INVALID_CONFIGURATION,
            "Cannot resolve realpath (" + e.getMessage() + ")");
      }
    }
  }
   ...
   *
   * <p>This helper method provides type-safe access to the serial port specific configuration
   * without requiring explicit casting throughout the code.
   *
   ...
   *
   * <ol>
   *   <li>Checks if the port is already open to avoid duplicate operations
   *   <li>Creates a new SerialPort handler if needed
   *   <li>Opens the port using the resolved port name
   *   <li>Configures the port with the specified communication parameters
   *   <li>Adds event listener if configured for event-driven mode
   *   <li>Sets the channel status to STARTED
   * </ol>
   *
   * <p>If any step fails, the method logs the error, handles specific error conditions (like
   * PORT_BUSY), and ensures the channel enters an appropriate error state.
   *
   * @see #closePort()
   * @see #findPortName()
   * @see #hasEventListener()
  ...
  protected void openPort() {
    ...
    try {
      ...
      if (!this.portHandler.setParams(
          this.getBaudrate(),
          this.getDataBits(),
          stopBitsFromConfiguration(this.getStopBits()),
          parityFromConfiguration(this.getParity()))) {
        this.logger.error(
            "Channel "
                + this.getName()
      ...
      if (exception.getExceptionType().equals(SerialPortException.TYPE_PORT_BUSY)) {
        this.notifyOnErrorDetected(
            ChannelSerialPortErrorCode.PORT_BUSY.getCode(ERROR_CODE_OFFSET), errorMessage);
      }
      this.closePortForced();
      return;
    }
   ...
   *
   * <ol>
   *   <li>Checks if the port is already closed to avoid duplicate operations
   *   <li>Removes event listener if one was configured
   *   <li>Closes the port and releases system resources
   *   <li>Sets the channel status to STOPPED
   * </ol>
   *
   * <p>If any step fails, the method logs the error but continues with the remaining cleanup steps
   * to ensure the channel is in a consistent state.
   *
   * @see #openPort()
   * @see #closePortForced()
   * @see #hasEventListener()
   ...
   *
   * <p>This method performs an emergency closure of the serial port connection without the normal
   * cleanup procedures. It attempts to close the port and then creates a new SerialPort handler
   * instance for future use.
   *
   * <p>This method is typically used in error conditions where the normal close procedure may fail
   * or when the port needs to be forcibly reset.
   *
   * <p>Unlike {@link #closePort()}, this method does not remove event listeners or update the
   * channel status, making it suitable for error recovery scenarios.
   *
   * @see #closePort()
   * @see #findPortName()
   */
   ...
   *
   * <p>This method uses the current channel configuration to determine if the specified port is
   * available. It delegates to the static {@link #isPortFound(String, String)} method with the
   * current port ID and port name.
   *
   ...
   *
   * <p>This method compares the name of the currently opened port with the port name that should be
   * opened according to the current configuration. This is useful for detecting situations where
   * the port has changed or become unavailable.
   *
   ...
   *
   * <p>This method is called when the system cannot locate the configured serial port. It logs the
   * error, sets the channel status to ERROR, forcibly closes any existing connection, and notifies
   * listeners of the error condition.
   *
   * <p>This typically occurs when a USB device is unplugged or the device driver is not properly
   * installed.
   *
   * @see #isPortFound()
  ...
  protected void onPortNotFound() {
    String errorMessage =
        "Channel " + this.getName() + " : serial port " + this.findPortName() + " not found";
    this.logger.error(errorMessage);
    this.setStatus(ChannelStatus.ERROR);
    this.closePortForced();
    this.notifyOnErrorDetected(
        ChannelSerialPortErrorCode.PORT_NOT_FOUND.getCode(ERROR_CODE_OFFSET), errorMessage);
  }

  /**
   * Handles the event when the currently opened port is no longer found.
   *
   * <p>This method is called when the system can no longer locate the port that was previously
   * opened by the channel. This indicates that the device has been physically disconnected or the
   * system has lost access to it.
   *
   * <p>It logs the error, sets the channel status to ERROR, forcibly closes the connection, and
   * notifies listeners of the error condition.
   *
  ...
  protected void onPortOpenedNotFound() {
    ...
    this.closePortForced();
    this.notifyOnErrorDetected(
        ChannelSerialPortErrorCode.PORT_OPENED_NOT_FOUND.getCode(ERROR_CODE_OFFSET), errorMessage);
  }
}
