...
public class CodecTICFrameStandard implements Codec<TICFrameStandard, byte[]>
{
	...
	public TICFrameStandard decode(byte[] bytes) throws CodecException
	{
		...
		if ((bytesArray.startsWith(TICFrame.BEGINNING_PATTERN) == true) && (bytesArray.endsWith(TICFrame.END_PATTERN) == true) && (bytesArray.contains(TICFrame.EOT) == false))
		{
			bytesArray.remove(0);
			...
			// If the format of a zone is invalid, then the browse is interrupted and the whole frame is invalid
			// (null)
