 ...
 *
 * <p>This codec handles the encoding and decoding of TIC Historic frames, which are used in the
 * French electricity meter communication protocol. Historic frames contain historical consumption
 * data and are part of the TIC protocol specification.
 *
 * <p>The codec processes byte arrays containing TIC Historic frame data and converts them to
 * TICFrameHistoric objects, and vice versa. It validates frame structure, handles data set parsing,
 * and ensures proper checksum validation.
 *
...
public class CodecTICFrameHistoric implements Codec<TICFrameHistoric, byte[]> {
  ...
  /**
   * Separator character used in TIC Historic frames. This is the space character (0x20) used to
   * separate different parts of the frame structure in the TIC protocol.
   */
   ...
   *
   * <p>This method validates the frame structure by checking for proper beginning and end patterns,
   * extracts individual data sets from the frame, and decodes each data set using the
   * CodecTICFrameHistoricDataSet codec. If any data set fails to decode, the error is collected and
   * thrown as a CodecException.
   *
  ...
  public TICFrameHistoric decode(byte[] bytesBuffer) throws CodecException {
    ...
    if ((true == bytesArray.startsWith(TICFrame.BEGINNING_PATTERN))
        && (true == bytesArray.endsWith(TICFrame.END_PATTERN))
   ...
   *
   * <p>This method takes a TICFrameHistoric object and converts it into the byte representation of
   * a TIC Historic frame. It processes each data set in the frame, encodes them using the
