...
public class BytesArray implements List<Byte>
{
	...
	public static byte[] intToArrayOfByte(int value, int numberOfByte) throws NumberFormatException
	{
		...
		{
			throw new NumberFormatException("Value " + value + " can't be converted in a byte[" + numberOfByte + "]");
		}
	...
	public void add(int index, Byte element)
	{
		...
		else
		{
			...
			else
			{
				...
				System.arraycopy(this.buffer, index, // src , src_pos
						bytesArray, index + 1, // dest, dest_pos
	...
	public boolean addAll(int index, Collection<? extends Byte> collection)
	{
		...
		if ((null != collection) && (collection.size() > 0) && (index >= 0) && (index <= this.buffer.length))
		{
			Object[] collectionArray = collection.toArray();
			byte[] bytesArray = new byte[this.buffer.length + collection.size()];
			...
			else
			{
				...
				System.arraycopy(this.buffer, index, bytesArray, index + collectionArray.length, this.buffer.length - index);
			}
	...
	public int indexOf(Object element, int offset)
	{
		...
		if ((null != element) && (element instanceof Number) && offset >= 0 && offset < this.buffer.length)
		{
	...
	public List<BytesArray> split(Number pattern)
	{
		...
		if (false == patternsIndexesList.isEmpty())
		{
			...
			{
				end_index = (i < patternsIndexesList.size()) ? patternsIndexesList.get(i) : this.buffer.length;
	 ...
	 * @return slices list
	 */
	public List<BytesArray> slice(Number beginPattern, Number endPattern, int occurences, int options)
	{
		List<BytesArray> bytesArraysList;

		// ... Si l'appel de la fonction est "gourmand" (greedy), il n'y aura au mieux qu'un élément à retourner qui
		...
		// ... Si l'appel de la fonction est "non gourmand" (not greedy), les éléments du tableau à retourner
	...
	public boolean addAll(int index, byte[] bytesArray)
	{
		...
		if ((index >= 0) && (index <= this.buffer.length))
		{
			...
			else
			{
				...
				System.arraycopy(this.buffer, index, newBytesArray, index + bytesArray.length, this.buffer.length - index);
			}
	 ...
	 * @return
	 */
	private List<BytesArray> sliceNotGreedy(Number beginPattern, Number endPattern, int occurences, int options)
	{
		List<BytesArray> bytesArraysList = new ArrayList<BytesArray>();
	 ...
	 * @return
	 */
	private List<BytesArray> sliceContiguous(Number beginPattern, Number endPattern, int occurences, int options)
	{
		List<BytesArray> bytesArraysList = new ArrayList<BytesArray>();
	 ...
	 * @return
	 */
	private List<BytesArray> sliceNotContiguous(Number beginPattern, Number endPattern, int occurences, int options)
	{
		List<BytesArray> bytesArraysList = new ArrayList<BytesArray>();
	 ...
	 * @param options
	 */
	private void slicePart(List<BytesArray> bytesArraysList, int beginIndex, int endIndex, int options)
	{
		if (beginIndex >= 0)
		{
			if ((options & REMOVE_PATTERNS) != 0)
	...
	private void removeIndex(int index)
	{
		...
		else
		{
			...
			System.arraycopy(this.buffer, index + 1, // src , src_pos
					bytesArray, index, // dest, dest_pos
	...
	private void removeSubList(int fromIndex, int toIndex)
	{
		...
		{
			System.arraycopy(this.buffer, toIndex + 1, bytesArray, fromIndex, bytesArray.length - fromIndex);
		}
