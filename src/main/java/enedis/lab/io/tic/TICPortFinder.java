...
public interface TICPortFinder extends PortFinder<TICPortDescriptor>
{
	...
	{
		return this.findAll().stream().filter(p -> (p.getPortId() != null) ? p.getPortId().equals(portId) : portId == null).findFirst().orElse(null);
	}
	...
	{
		return this.findAll().stream().filter(p -> (p.getPortName() != null) ? p.getPortName().equals(portName) : portName == null).findFirst().orElse(null);
	}
	...
	public default TICPortDescriptor findByPortIdOrPortName(String portId, String portName)
	{
		...
		if (descriptor == null)
		{
			descriptor = this.findByPortName(portId);
		}
