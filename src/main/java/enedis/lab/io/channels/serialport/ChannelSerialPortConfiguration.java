 ...
 *
 * <p>This class extends {@link ChannelConfiguration} and provides specific configuration parameters
 * for serial port communication, including port identification, baud rate, parity settings, data
 * bits, stop bits, and timeout configurations.
 *
 * <p>The configuration supports both port ID and port name identification methods, with validation
 * for standard serial communication parameters.
 *
 * <p>Supported baud rates: 110, 300, 1200, 2400, 4800, 9600, 19200, 38400, 57600, 115200, 230400,
 * 460800, 921600, 1843200, 3686400
 *
 * <p>Supported data bits: 5, 6, 7, 8
 *
 * <p>Supported stop bits: 1.0, 1.5, 2.0
 *
 * <p>Default sync read timeout: 10000 milliseconds
 *
...
public class ChannelSerialPortConfiguration extends ChannelConfiguration {
  ...
  private static final Number[] BAUDRATE_ACCEPTED_VALUES = {
    110, 300, 1200, 2400, 4800, 9600, 19200, 38400, 57600, 115200, 230400, 460800, 921600, 1843200,
    3686400
  };
   ...
   *     incompatible with serial port configuration
   */
  ...
  /**
   * Constructs a new serial port configuration with a name and file reference, initializing all
   ...
   *
   * <p>This constructor provides a convenient way to create a fully configured serial port channel
   * with all necessary parameters specified at construction time.
   *
   * @param name the configuration name
   ...
   */
  public ChannelSerialPortConfiguration(
      String name,
      String alias,
      String portId,
      String portName,
   ...
   *
   * <p>This method ensures that either port ID or port name is specified, and sets default values
   * for protocol, direction, and sync read timeout if they are not already configured.
   *
   * @throws DataDictionaryException if neither port ID nor port name is defined, or if there are
   *     validation errors with the configuration
   */
  @Override
  protected void updateOptionalParameters() throws DataDictionaryException {
    if (!this.exists(KEY_PORT_ID) && !this.exists(KEY_PORT_NAME)) {
      throw new DataDictionaryException(
          "Neither " + KEY_PORT_ID + " nor " + KEY_PORT_NAME + " is defined");
    }
   ...
   *
   * <p>The port ID is an alternative way to identify the serial port, typically used when the
   * system provides a unique identifier for the port.
   *
   ...
   *
   * <p>The port name is typically a system-specific identifier such as "/dev/ttyUSB0" on Unix
   * systems or "COM1" on Windows systems.
   *
   ...
   *
   * <p>The baud rate determines the speed of data transmission over the serial port.
   *
   ...
   *
   * <p>Parity is used for error detection in serial communication. It can be NONE, ODD, or EVEN
   ...
   *
   * <p>Data bits determine how many bits are used to represent each character in the serial
   * communication. Common values are 7 or 8 bits.
   *
   ...
   *
   * <p>Stop bits are used to signal the end of a data frame in serial communication.
   *
   ...
   *
   * <p>This timeout determines how long the system will wait for data to be available on the serial
   * port before timing out the read operation.
   *
   ...
   *
   * <p>The port ID provides an alternative way to identify the serial port, typically used when the
   * system provides a unique identifier for the port. Either port ID or port name must be specified
   * for a valid configuration.
   *
   * @param portId the port identifier to set
   * @throws DataDictionaryException if the port ID is invalid or conflicts with other configuration
   *     parameters
   */
   ...
   *
   * <p>The port name is typically a system-specific identifier such as "/dev/ttyUSB0" on Unix
   * systems or "COM1" on Windows systems. Either port ID or port name must be specified for a valid
   * configuration.
   *
   * @param portName the port name to set
   * @throws DataDictionaryException if the port name is invalid or conflicts with other
   ...
   *
   * <p>The baud rate determines the speed of data transmission over the serial port. Must be one of
   * the supported values: {@link #BAUDRATE_ACCEPTED_VALUES}.
   *
   ...
   *
   * <p>Parity is used for error detection in serial communication. The parity setting must be one
   * of the supported {@link Parity} enum values.
   *
   ...
   *
   * <p>Data bits determine how many bits are used to represent each character in the serial
   * communication. Must be one of: {@link #DATA_BITS_ACCEPTED_VALUES}.
   *
   ...
   *
   * <p>Stop bits are used to signal the end of a data frame in serial communication. Must be one
   * of: {@link #STOP_BITS_ACCEPTED_VALUES}.
   *
   ...
   *
   * <p>This timeout determines how long the system will wait for data to be available on the serial
   * port before timing out the read operation. If not set, defaults to {@link
   * #SYNC_READ_TIMEOUT_DEFAULT_VALUE}.
   *
   ...
   *
   * <p>This method sets up the validation rules for all serial port configuration parameters,
   * including accepted values and required/optional status.
   *
