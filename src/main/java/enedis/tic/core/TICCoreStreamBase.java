...
public class TICCoreStreamBase implements TICCoreStream, Task, DataStreamListener
{
	...
	public TICCoreStreamBase(String portId, String portName, TICMode ticMode) throws TICCoreException
	{
		...
		if (portId != null)
		{
			...
			if (descriptor == null)
			{
				TICCoreException exception = new TICCoreException(TICCoreErrorCode.STREAM_PORT_ID_NOT_FOUND.getCode(), "TICCore stream port id " + portId + " not found!");
		...
		else if (portName != null)
		{
			...
			if (descriptor == null)
			{
				...
				if (descriptor == null)
				{
					TICCoreException exception = new TICCoreException(TICCoreErrorCode.STREAM_PORT_NAME_NOT_FOUND.getCode(),
		...
		{
			TICCoreException exception = new TICCoreException(TICCoreErrorCode.STREAM_PORT_DESCRIPTOR_EMPTY.getCode(), "TICCore stream port descriptor empty!");
			logger.error(exception.getMessage(), exception);
			throw exception;
		}
		if (ticMode == null)
		{
			TICCoreException exception = new TICCoreException(TICCoreErrorCode.STREAM_MODE_NOT_DEFINED.getCode(), "TICCore stream mode not defined!");
			logger.error(exception.getMessage(), exception);
			throw exception;
		}
		try
		{
			...
			this.channel = new ChannelTICSerialPort(new ChannelTICSerialPortConfiguration("Channel@" + descriptor.getPortName(), portName, ticMode));
			this.stream = new TICInputStream(
					new TICStreamConfiguration("Stream@" + descriptor.getPortName(), DataStreamDirection.INPUT, "Channel@" + descriptor.getPortName(), ticMode));
		...
		{
			TICCoreException exception = new TICCoreException(TICCoreErrorCode.OTHER_REASON.getCode(), "TICCore stream instanciation failed : " + e.getMessage());
			logger.error(exception.getMessage(), exception);
			throw exception;
		}
	}
