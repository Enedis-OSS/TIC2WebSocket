...
public class TIC2WebSocketRequestHandlerBase implements TIC2WebSocketRequestHandler
{
	...
	public Response handle(Request request, TIC2WebSocketClient client)
	{
		...
		switch (request.getName())
		{
			...
			default:
				...
				response = this.createErrorResponse(request.getName(), TIC2WebSocketEndPointErrorCode.UNSUPPORTED_MESSAGE, "Request " + request.getName() + " not supported");
				break;
		}
	...
	private Response handleGetAvailableTICsRequest(RequestGetAvailableTICs request)
	{
		...
		try
		{
			response = new ResponseGetAvailableTICs(LocalDateTime.now(), TIC2WebSocketEndPointErrorCode.NO_ERROR.value(), null, ticIdentifiers);
		}
		catch (DataDictionaryException e)
		{
			this.logger.error(e.getMessage(), e);
			response = this.createErrorResponse(request.getName(), TIC2WebSocketEndPointErrorCode.INTERNAL_ERROR, e.getMessage());
		}
	...
	private Response handleGetModemsInfoRequest(RequestGetModemsInfo request)
	{
		...
		try
		{
			response = new ResponseGetModemsInfo(LocalDateTime.now(), TIC2WebSocketEndPointErrorCode.NO_ERROR.value(), null, modemsInfo);
		}
		catch (DataDictionaryException e)
		{
			this.logger.error(e.getMessage(), e);
	...
	private Response handleReadTICRequest(RequestReadTIC request)
	{
		...
		try
		{
			...
			response = new ResponseReadTIC(LocalDateTime.now(), TIC2WebSocketEndPointErrorCode.NO_ERROR.value(), null, frame);
		}
		catch (DataDictionaryException e)
		{
			this.logger.error(e.getMessage(), e);
		...
		catch (TICCoreException e)
		{
			...
			{
				response = this.createErrorResponse(request.getName(), TIC2WebSocketEndPointErrorCode.IDENTIFIER_NOT_FOUND, e.getMessage());
			}
			else if(e.getErrorCode() == TICCoreErrorCode.DATA_READ_TIMEOUT.getCode())
			{
				response = this.createErrorResponse(request.getName(), TIC2WebSocketEndPointErrorCode.READ_TIMEOUT, e.getMessage());
			}
			else
			{
				response = this.createErrorResponse(request.getName(), TIC2WebSocketEndPointErrorCode.INTERNAL_ERROR, e.getMessage());
	...
	private Response handleSubscribeTICRequest(RequestSubscribeTIC request, TIC2WebSocketClient client)
	{
		Response response = null;
		...
		{
			List<TICIdentifier> newSubscriptions = this.getNewSubcriptions(this.ticCore.getIndentifiers(client), ticIdentifiers.get());
			if(!newSubscriptions.isEmpty())
			{
				for(TICIdentifier identifier : ticIdentifiers.get())
				{
					try
					{
						...
						try
						{
							response = new ResponseSubscribeTIC(LocalDateTime.now(), TIC2WebSocketEndPointErrorCode.NO_ERROR.value(), null);
						}
						catch (DataDictionaryException e)
						{
							this.logger.error(e.getMessage(), e);
					...
					{
						response = this.createErrorResponse(request.getName(), TIC2WebSocketEndPointErrorCode.SUBSCRIPTION_FAIL, e.getMessage());
					}
				}
			}
		}
	...
	private List<TICIdentifier> getNewSubcriptions(List<TICIdentifier> currentSubscriptions, List<TICIdentifier> askedTicIdentifiers)
	{
	...
	private Response handleUnsubscribeTICRequest(RequestUnsubscribeTIC request, TIC2WebSocketClient client)
	{
		Response response = null;
		Optional<List<TICIdentifier>> ticIdentifiers = Optional.ofNullable(request.getData());
		...
		if (ticIdentifiers.isPresent())
		{
			for(TICIdentifier identifier : ticIdentifiers.get())
			{
				try
				{
					...
					try
					{
						response = new ResponseUnsubscribeTIC(LocalDateTime.now(), TIC2WebSocketEndPointErrorCode.NO_ERROR.value(), null);
					}
					catch (DataDictionaryException e)
					{
						this.logger.error(e.getMessage(), e);
				...
				{
					response = this.createErrorResponse(request.getName(), TIC2WebSocketEndPointErrorCode.UNSUBSCRIPTION_FAIL, e.getMessage());
				}
			}
		}
		else
	...
	private Response createErrorResponse(String name, TIC2WebSocketEndPointErrorCode code, String message)
	{
		try
		{
