...
public class TIC2WebSocketConfiguration extends ConfigurationBase
{
	 ...
	 * @throws DataDictionaryException
	 */
	public TIC2WebSocketConfiguration(Number serverPort, TICMode ticMode, List<String> ticPortNames) throws DataDictionaryException
	{
		this();
	...
	protected void setTicPortNames(Object ticPortNames) throws DataDictionaryException
	{
		...
		for (int i = 0; i < portNames.size(); i++)
		{
			...
			if (portName == null)
			{
				throw new DataDictionaryException("Key " + KEY_TIC_PORT_NAMES + ": value at index " + i + "(" + portName + ") cannot be null");
			}
			else if (portName.isEmpty())
			{
				throw new DataDictionaryException("Key " + KEY_TIC_PORT_NAMES + ": value at index " + i + "(" + portName + ") cannot be empty");
			}
			else if (i != portNames.lastIndexOf(portName))
			{
				throw new DataDictionaryException(
						"Key " + KEY_TIC_PORT_NAMES + ": value at index " + i + " and " + portNames.lastIndexOf(portName) + "(" + portName + ") are identical");
			}
		}
	...
	private void loadKeyDescriptors()
	{
		try
		{
			this.kServerPort = new KeyDescriptorNumberMinMax(KEY_SERVER_PORT, true, SERVER_PORT_MIN, SERVER_PORT_MAX);
			...
			this.kTicPortNames = new KeyDescriptorListMinMaxSize<String>(KEY_TIC_PORT_NAMES, false, String.class, TIC_PORT_NAMES_MIN_SIZE, null);
