...
/**
 * Generic base implementation for managing filtered notifications to
 * subscribers.
 *
 * <p>
 * This class provides mechanisms for subscribing, unsubscribing, and querying
 * subscribers with
 * associated filters. It supports both filtered and unfiltered (global)
 * subscriptions, enabling
...
public class FilteredNotifierBase<F, T extends Subscriber> implements FilteredNotifier<F, T> {
    ...
    public Collection<T> getSubscribers(F filter) {
        this.subscribersFilteredLock.lock();
        Collection<T> subscribersFiltered = (this.hasFilter(filter)) ? this.subscribersFiltered.get(filter)
                : new HashSet<T>();
        this.subscribersFilteredLock.unlock();
