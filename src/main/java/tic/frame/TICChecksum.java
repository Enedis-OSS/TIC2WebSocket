...
public class TICChecksum {
   ...
   */
  public static byte computeChecksum(byte[] data, TICMode mode) {
    int crc = 0;
		for(int i = OFFSET_BEGIN; i < data.length - 3; i++)
		{
			...
			{
				if(data[i] == TICSeparator.HISTORIC.getHexValue())
				{
					mode = TICMode.HISTORIC;
				}
				else if(data[i] == TICSeparator.STANDARD.getHexValue())
				{
